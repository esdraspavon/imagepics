...
window.addEventListener('load', () => {
	...
	if (settings.has('cloudup.passwd')) {
		const decipher = crypto.createDecipher('aes192', 'Imagepics')
...
function saveButton () {
	...
	saveButton.addEventListener('click', () =>{
		if (prefsForm.reportValidity()) {
			const cipher = crypto.createCipher('aes192', 'Imagepics')
			...
			ipcRenderer.send('show-dialog', { type: 'error', title: 'Imagepics', message: 'Por favor complete los campos requeridos'})
		}
	})
}